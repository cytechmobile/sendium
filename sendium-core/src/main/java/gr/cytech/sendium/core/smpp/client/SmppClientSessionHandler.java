...
public class SmppClientSessionHandler implements SmsgSmppSessionHandler {
    public SmppSession session;
    public long lastPduTimestamp;
    public long sessionStartTime;

    protected final SmppClientWorker.ConnectionInfo info;
    protected final SmppClientWorker smppClientWorker;
    protected final Logger logger;
    protected int consecutiveFailedEnquireLinks;

    public SmppClientSessionHandler(SmppClientWorker smppClientWorker, SmppClientWorker.ConnectionInfo info) {
        this.smppClientWorker = smppClientWorker;
        this.logger = LoggerFactory.getLogger(
                SmppClientSessionHandler.class.getCanonicalName() + "." + smppClientWorker.getFullName());
    ...
    public void configMaxRate(String accountId) {
        //no-op, exists for compatibility with the server handler
    }

    @Override
    public double getRate() {
        return smppClientWorker.getTransactionsPerSecond();
    }
    ...
    public boolean isBackupConnection() {
        return info.type() == SmppClientWorker.ConnectionType.BACKUP;
    }

    public SmppClientWorker.ConnectionInfo getConnectionInfo() {
        return info;
    }
    ...
    public PduResponse firePduRequestReceived(PduRequest pduRequest) {
        ...
        switch (pduRequest.getCommandId()) {
            case SmppConstants.CMD_ID_DELIVER_SM:
                ...
                } else if (SmppUtil.isMessageTypeIntermediateDeliveryReceipt(deliverSm.getEsmClass())) {
                    resp = deliverSm.createResponse();
                    // create an empty success response so the carrier knows we got it,
                    // but DO NOT send it to parseDlr or parseMo.
                } else {
            ...
            case SmppConstants.CMD_ID_ENQUIRE_LINK:
                ...
                resp.setReferenceObject(this);
                logger.trace("responding to enquire link request:{} with resp:{}", pduRequest, resp);
                break;
            case SmppConstants.CMD_ID_UNBIND:
                //TODO: how to handle this? just checking for connections won't cut it
                //should I remove this connection up front?
    ...
    public void fireExpectedPduResponseReceived(PduAsyncResponse pduAsyncResponse) {
        ...
        switch (pduAsyncResponse.getRequest().getCommandId()) {
            case SmppConstants.CMD_ID_SUBMIT_SM:
                ...
                break;
            case SmppConstants.CMD_ID_ENQUIRE_LINK:
                if (pduAsyncResponse.getRequest().getCommandStatus() == SmppConstants.STATUS_OK) {
                    handleSuccessEnquireLink();
                } else if (pduAsyncResponse.getRequest().getCommandStatus() != SmppConstants.STATUS_OK) {
                    handleFailedEnquireLink();
                }
                break;
    ...
    public void handleFailedEnquireLink() {
        ...
        int maxErrors = smppClientWorker.getMaxConsecutiveFailedEnquireLinksBeforeReconnecting();
        if (maxErrors > 0 && this.consecutiveFailedEnquireLinks >= maxErrors) {
            logger.warn("removing connection:{} due to reaching maximum consecutive number of failed enquire links: {}",
                    this, maxErrors);
            smppClientWorker.removeConnection(this, true, true);
        }
    }
    ...
    public String toString() {
        return smppClientWorker.getFullName() + ".SessionHandler{" +
                (session == null ?
                        "session=<NO SESSION>" :
                        "session.bindType=" + session.getBindType() +
                                ", session.status=" + session.getStateName()) +
                '}';
    }
}
