...
const KEY_CATALOG = {
  smsc: {
    ...
    manual: new Map([
      ...
      ['denied-smsc-id', 'Kannel SMSC deny routing has no direct MVP mapping.'],
      ...
      ['wait-ack', 'Kannel acknowledgement behavior has no direct MVP mapping.'],
      ...
      ['bind-addr-ton', 'Bind address TON is not mapped by the MVP.'],
      ['bind-addr-npi', 'Bind address NPI is not mapped by the MVP.'],
    ]),
  },
  'sendsms-user': {
    ...
    .forEach((group) => {
      ...
      if (protocol && protocol.toLowerCase() !== 'smpp') {
        diagnostics.push({
          ...
          line: group.line,
          group: group.type,
          message: `Only SMPP SMSC blocks are converted in the MVP. Found \`smsc = ${protocol}\`.`,
        });
        diagnoseCatalogKeys(group, diagnostics);
        return;
      }
  ...
  groups
    ...
    .forEach((group) => {
      diagnostics.push({
        ...
        line: group.line,
        group: group.type,
        message: `Kannel \`group = ${group.type}\` is not converted by the MVP.`,
      });
      diagnoseCatalogKeys(group, diagnostics);
    });
...
function renderRoutingTable(clients, smppServers = []) {
  ...
  return [
    ...
    '# Kannel routing is not deeply converted in the MVP. Review docs/05-routing-engine.md.',
    '[default]',
    'MESSAGE:type:==:0',
    'MESSAGE:type:==:11',
    'MESSAGE:type:==:14',
...
function diagnoseCatalogKeys(group, diagnostics) {
  ...
  group.entries.forEach((entry) => {
    ...
    line: entry.line,
      group: group.type,
      key: entry.key,
      message: manualMessage || `Kannel \`${entry.key}\` is not in the MVP mapping catalog and needs manual review.`,
    });
  });
}
