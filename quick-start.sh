...
stage_upstream_activation=false
if [ "$force" = true ] && [ "$upstream_enabled" = true ]; then
    stage_upstream_activation=true
    sed 's/^outSms\.instance\.upstream\.enable = true$/outSms.instance.upstream.enable = false/' \
        "$staging_dir/conf/smsg.properties" > "$staging_dir/conf/smsg.properties.disabled"
fi
...
if [ "$stage_upstream_activation" = true ]; then
    chmod 600 "$staging_dir/conf/smsg.properties.disabled"
fi
...
if [ "$stage_upstream_activation" = true ]; then
    mv -f "$staging_dir/conf/smsg.properties.disabled" "$target_dir/conf/smsg.properties"
else
...
if [ "$stage_upstream_activation" = true ]; then
    # Let a running instance load every worker property before enabling it.
    sleep 1
    mv -f "$staging_dir/conf/smsg.properties" "$target_dir/conf/smsg.properties"
fi
