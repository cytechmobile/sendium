...
staged_dir="$test_root/staged-activation"
sh "$quick_start" --directory "$staged_dir" --provider local --no-start > "$test_root/staged-local.out" 2>&1
mkdir -p "$test_root/fake-bin"
cat > "$test_root/fake-bin/sleep" <<'EOF'
#!/bin/sh
cp "$SENDIUM_STAGE_SOURCE" "$SENDIUM_STAGE_CAPTURE"
EOF
chmod 700 "$test_root/fake-bin/sleep"
SENDIUM_UPSTREAM_USERNAME='prosms-user' \
SENDIUM_UPSTREAM_PASSWORD='pass1234' \
SENDIUM_STAGE_SOURCE="$staged_dir/conf/smsg.properties" \
SENDIUM_STAGE_CAPTURE="$test_root/staged-disabled.properties" \
PATH="$test_root/fake-bin:$PATH" \
    sh "$quick_start" --directory "$staged_dir" --provider prosms --force --no-start > "$test_root/staged-force.out" 2>&1
assert_contains 'outSms.instance.upstream.enable = false' "$test_root/staged-disabled.properties"
assert_contains 'outSms.instance.upstream.type = smppclient' "$test_root/staged-disabled.properties"
assert_contains 'outSms.instance.upstream.enable = true' "$staged_dir/conf/smsg.properties"
assert_not_contains 'outSms.instance.upstream.enable = false' "$staged_dir/conf/smsg.properties"
pass "staged upstream activation on regeneration"
...
if [ -n "${SENDIUM_TEST_IMAGE-}" ]; then
    ...
    pass "forced upstream activation in a running container"
else
